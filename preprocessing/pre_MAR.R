# Mass accumulation rates (MAR calculations)
...
#bulk5<-mutate(bulk5, DBD= (Wet.sediment.bag.g.-Totaldrysediment)/Volume)
bulk5<-mutate(bulk5, DBD= (Wet.sediment.bag.g.-Dry.sediment..g..bag)/Volume)
bulk5<-mutate(bulk5, water_g= (Wet.sediment.bag.g.-Dry.sediment..g..bag))
#bulk5<-mutate(bulk5, minus_2= ifelse(aluminium. =="y", (minus-Bag..g.-Aluminium.paper),(minus-Bag..g.)))
bulk5<-mutate(bulk5, ms= (Wet.sediment.bag.g.-water_g))
bulk5<-mutate(bulk5, ms_2= ifelse(aluminium. =="y", (ms-Bag..g.-Aluminium.paper),(ms-Bag..g.)))

bulk55<-select(bulk5, Identifier, DBD_2, Dry_bulk_density)

#bulk5<-mutate(bulk5, minus_3= ms-Bag..g.)


bulk5<-mutate(bulk5, DBD_2= (ms_2/Volume))

#Calculation of bulk density ######Values needed to be corrected - no missing bc need to be  used for PYC calcs

bulk5<-mutate (bulk5, Dry_bulk_density=((Totaldrysediment/Volume))) #  g/cm3
...
merged.datamass3<-mutate(merged.datamass3,sedrate.mm=(1/yearbetsamples)*10) #mm/yr


#Calculate Mass acc rate
merged.datamass4<-mutate(merged.datamass3,MAR=(Dry_bulk_density2*sedrate)*1000) #so it is in mg (g/cm3)*cm/yr
...
MAR_3<-ggplot(MAR_2, aes(x=median,y=MAR_2))+geom_point() +  scale_x_continuous(breaks = seq(0, 32500, by = 2500)) + ggtitle("")+ labs(x= expression(bold(delta^13)),y = "MAR (g cm-2 yr-1)")+theme_bw() + xlab("Age (cal kyr BP)")+ theme(axis.text.x=element_text(size=12),axis.title.x=element_text(size=12,face="bold"),axis.title.y = element_text(size=12,face="bold"))
MAR_3
